...
release on this fork, with notes generated by `git-cliff`. The checksum covers the tarball under its bare name, so
verifying a download is `sha256sum -c zellij-x86_64-unknown-linux-musl.sha256sum`
in the directory both files landed in. Upstream's own releases hash the unpacked
binary instead, which is why the two are not interchangeable.
...
asking `git-cliff` for the latest tag, because fork tags and mirrored upstream
tags sit on branches that diverged, so ordering them by date interleaves them
and would make a release re-list everything the one before it already shipped. The artifact carries upstream's own naming so it drops
into anything that already unpacks a zellij release; the notes are what state
that this is a fork build and name the exact commit it came from, since the
release page is the only context a reader gets.
...
run for a build you install on one machine. Adding a target means adding a
matrix entry alongside the existing build step.
